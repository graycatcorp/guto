...
- **Produtos**: centros de custo de Chat, Documentos, Estuda, Música, Design e Esportes. O topo reúne barras de cenário para base, operações e simulação; a coorte usa seletor segmentado e, na Base mista, um percentual premium explícito (5% inicial). Anúncios incidem somente na parcela gratuita e receita premium somente nos usuários premium. O modo Manual recebe ativos/operações diretamente; o **Mundo simulado** transforma uma base total Tupham em ativos e operações usando premissas específicas por feature. Os seletores de feature exibem somente o saldo e um marcador verde/vermelho; os indicadores consolidados ficam em carrossel horizontal. Cada centro usa o mesmo chassi analítico: artefatos e receitas em painéis gêmeos com expansão por `+`, dependências, custos fixos e variáveis, cotas, payback e gráfico receita × custo com atalhos de feature/base/operações. Premissas financeiras persistem no estado compartilhado.
- **Chat offline**: o modelo Tupico/GGUF já vem embarcado no bundle do app e roda via llama.cpp no aparelho; não depende de Cloudflare Worker/R2 nem possui custo separado de distribuição do modelo.
