...
- **Produtos**: centros de custo de Chat, Documentos, Estuda, Música, Design e Esportes. O topo reúne quatro widgets de cenário (base, operações, simulação e coorte). O modo Manual recebe ativos/operações diretamente; o **Mundo simulado** transforma uma base total Tupham em ativos e operações usando premissas específicas por feature. Cada centro usa o mesmo chassi analítico: artefatos com custo efetivo e detalhamento clicável por provedor/serviço, receitas em painel gêmeo, dependências, custos fixos e variáveis, cotas, payback e gráfico receita × custo com atalhos de feature/base/operações. Premissas financeiras persistem no estado compartilhado.
